...
**状态：软件闭环已实现并通过静态测试；待板端拉取及 STM32 烧录后实车逐条验收**

- [x] V5 `01`～`30` 命令采用唯一映射并拒绝未知协议帧。
- [x] Linux 主控按执行状态下发 `80`～`DA` 被动播报请求。
- [x] 语音急停只允许锁定，不能通过语音解除；微动进入现有速度仲裁。
- [x] 单点、任务链、巡检和仪表识别复用统一 Mission 闭环。
- [x] 自动建图、模式切换、相机、地图保存和状态查询接入既有权威节点。
- [x] STM32 增加 USART1 播报帧到 UART5 的非阻塞队列转发。
- [ ] 烧录 V5 STM32 固件，逐条验证 48 个命令和关键状态播报。
- [ ] 将真实 `home` 与点位 1～4 写入 `inspection_route.json`，禁止保留示例坐标投入运行。

详细链路、安全约束与验收步骤见 `docs/VOICE_CONTROL_V5.md`。
